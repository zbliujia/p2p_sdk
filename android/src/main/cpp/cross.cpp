...
Java_com_cross_Cross_sdkinit(JNIEnv *env, jclass clazz, jstring j_token) {
    const char *token = env->GetStringUTFChars(j_token, JNI_FALSE);
    auto result = JZSDK_Init(token);
    env->ReleaseStringUTFChars(j_url, url);
    return result;
}